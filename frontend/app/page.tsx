...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => { setMyWaitTime(prev => prev > 0 ? prev - 1 : 0); }, 1000);
      ...
      <div className="w-full max-w-md flex justify-between items-center mb-6 px-2">
          ...
          {currentUser ? (
              <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => { localStorage.removeItem('slotSync_user'); setCurrentUser(null); }} className="text-xs text-red-500 border border-red-900/50 px-3 py-1 rounded-full">Logout</button>
              </div>
      ...
      <div className="w-full max-w-md bg-neutral-900 rounded-3xl border border-white/10 mb-8 relative overflow-hidden">
        {view === "home" && (
            <div className="p-10 text-center">
                ...
                {!amIInQueue && (
                    <button onClick={() => currentUser ? setView("join") : setView("login")} className="w-full bg-green-500 text-black font-black py-4 rounded-xl text-lg hover:bg-green-400 transition-all">
        ...
        {view === "login" && (
            <div className="p-8">
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                    <input className="w-full p-4 bg-black border border-white/10 rounded-xl text-white outline-none" placeholder="Username" value={form.username} onChange={e => setForm({...form, username: e.target.value})} />
                    <input type="password" className="w-full p-4 bg-black border border-white/10 rounded-xl text-white outline-none" placeholder="Password" value={form.password} onChange={e => setForm({...form, password: e.target.value})} />
        ...
        {view === "signup" && (
            <div className="p-8">
                ...
                <form onSubmit={handleSignup} className="space-y-3">
                    <input className="w-full p-3 bg-black border border-white/10 rounded-xl text-white outline-none" placeholder="Full Name" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
                    <input className="w-full p-3 bg-black border border-white/10 rounded-xl text-white outline-none" placeholder="Phone" maxLength={10} value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} />
                    <input className="w-full p-3 bg-black border border-white/10 rounded-xl text-white outline-none" placeholder="Username" value={form.username} onChange={e => setForm({...form, username: e.target.value})} />
                    <input type="password" className="w-full p-3 bg-black border border-white/10 rounded-xl text-white outline-none" placeholder="Password" value={form.password} onChange={e => setForm({...form, password: e.target.value})} />
                    {error && <p className="text-red-500 text-xs">{error}</p>}
        ...
        {view === "join" && (
             <div className="p-8">
                ...
                <div className="grid grid-cols-2 gap-2 mb-6">
                    {SERVICES.map(s => (
                        <button key={s.name} onClick={() => toggleService(s.name)} className={`p-3 rounded-xl text-sm font-bold border ${selected.includes(s.name) ? 'bg-green-500 text-black border-green-500' : 'bg-neutral-800 text-gray-400 border-white/5'}`}>
